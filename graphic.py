...
def y_hat(frequency, c, r):
    ...
    z_image = - (angular_frequency(frequency) * r**2 * c *
...
def gradient_descent(c, r, z_real_txt, z_image_txt, alpha):
    ...
    for i in range(len(frequency)):
        ...
        sc_1 = 2 * W * \
            (y_hat(frequency[i], c, r)[0] - z_real_txt[i]) * \
            numerador_c/denominador
        sc_2 = 2 * W * (y_hat(frequency[i], c, r)[1] - z_image_txt[i]) * (-angular_frequency(
            frequency[i]) * r**2*((-angular_frequency(frequency[i]) * r * c)**2 + 1))/denominador
        sc = sc_1 + sc_2
        # Resitor
        numerador_r = (-(angular_frequency(frequency[i]) * c * r)**2 + 1)
        sr_1 = 2 * W * \
            (y_hat(frequency[i], c, r)[0] - z_real_txt[i]) * \
            numerador_r/denominador
        sr_2 = 2 * W * (y_hat(frequency[i], c, r)[1] - z_image_txt[i]) * \
            (-2 * angular_frequency(frequency[i]) * r * c)/denominador
        sr = sr_1 + sr_2
